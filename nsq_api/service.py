...
class NsqAPI(tornado.web.RequestHandler):
    ...
    def _remove_channel(self, nsqd_http_address, topic, channel):
        """
        Delete a Nsq channel
        """
        requests.post(self.DELETE_CHANNEL_URL.format(nsqd_http_address, topic, channel))
    ...
    def get(self):
        ...
        nsqd_tcp_addresses, nsqd_http_address, topic, empty_lines = self._parse_query(
        ...
        if not nsqd_tcp_addresses:
            ...
            def cleanup():
                ...
                self._remove_channel(
                    nsqd_http_address=nsqd_http_address, topic=topic, channel=channel
                )
...
class NsqServer(BaseScript):
    ...
    def prepare_api(self):
        return tornado.web.Application([(r"/tail", NsqAPI, dict(log=self.log))])
    ...
    def start(self):
        ...
        if not self.master:
            ...
            if register_response.get("result", {}).get("success", {}):
                self.log.info("authentication_passed")
            else:
                raise Exception(register_response.get("result", {}).get("details", {}))
    ...
    def define_subcommands(self, subcommands):
        ...
        runserver_cmd = subcommands.add_parser("runserver", help="NsqServer Service")

        runserver_cmd.set_defaults(func=self.start)

        runserver_cmd.add_argument(
            "-p", "--port", default=1077, help="NsqServer port, default: %(default)s"
        )
        runserver_cmd.add_argument(
            "-i",
            "--host",
